...
// Generated-tree detection is specific to the docs aggregator: only
// tuna-os/docs has a sync that rewrites docs/<slug>/ from other repos'
// READMEs, and only there is "this prose is authored elsewhere" a reason to
// skip a file. Every other repo answers for all of its own prose.
//
// So sync-org-docs.mjs is an optional dependency, resolved at runtime against
// the repository being checked rather than imported statically. Absent, the
// generated set is empty and every tracked file is checked -- which is both
// the right default elsewhere and the safe direction: the failure mode is
// findings entering the budget, not prose going silently unchecked.
//
// This is deliberately a runtime lookup instead of a second copy of the file
// living beside the action. Two copies of this logic would drift, and the org
// already runs a workflow whose whole job is policing that class of drift.
let isSyncedIndex = null;
let HAND_AUTHORED = new Set();
try {
  const sync = await import(pathToFileURL(join(ROOT, 'scripts', 'sync-org-docs.mjs')).href);
  isSyncedIndex = sync.isSyncedIndex;
  HAND_AUTHORED = sync.HAND_AUTHORED;
} catch {
  // Not the aggregator -- no generated trees.
}
